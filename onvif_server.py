...
WS-Discovery (UDP 3702) + ONVIF Device/Media HTTP service.
Sends periodic Hello broadcasts to announce presence.
"""
...
def wsd_listener():
    ...
    # Send Hello on start and every 30s
    def hello_loop():
        while True:
            try:
...
<tds:GetCapabilitiesResponse>
  <tds:Capabilities>
    <tt:Media><tt:XAddr>{MEDIA_URL}</tt:XAddr><tt:StreamingCapabilities><tt:RTPMulticast>false</tt:RTPMulticast><tt:RTP_TCP>true</tt:RTP_TCP><tt:RTP_RTSP_TCP>true</tt:RTP_RTSP_TCP></tt:StreamingCapabilities></tt:Media>
  </tds:Capabilities>
</tds:GetCapabilitiesResponse>""")
...
<trt:GetProfilesResponse>
  <trt:Profiles token="profile1" fixed="true">
    ...
    <tt:VideoEncoderConfiguration token="vec1">
      ...
      <tt:RateControl><tt:FrameRateLimit>15</tt:FrameRateLimit><tt:EncodingInterval>1</tt:EncodingInterval><tt:BitrateLimit>4096</tt:BitrateLimit></tt:RateControl>
...
<trt:GetStreamUriResponse>
  <trt:MediaUri>
    ...
    <tt:Timeout>PT0S</tt:Timeout>
            ...
            print(f'[ONVIF HTTP] unhandled action: {body[:200]}', flush=True)
